...
contract SenditOld is EIP712Upgradeable, ReentrancyGuardUpgradeable {
  enum RequestStatus {
    OPEN,
    REJECTED,
    COMPLETED
  ...
  bytes32 private constant REQUEST_TYPEHASH =
    keccak256(
      "Request(uint256 nonce,address recipient,uint256 value,address token_address,uint256 expiry)"
    );
